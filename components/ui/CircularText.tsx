...
const CircularText = ({ text, spinDuration = 20, onHover = 'speedUp', className = '' }: CircularTextProps) => {
  ...
  return (
    <motion.div
      className={`relative mx-auto rounded-full w-[200px] h-[200px] text-white font-black text-center cursor-pointer origin-center ${className}`}
    ...
    >
      {letters.map((letter, i) => {
        ...
        return (
          <span 
            key={i} 
            className="absolute inset-0 inline-block text-[24px] transition-all duration-500 ease-[cubic-bezier(0,0,0,1)] origin-center"
